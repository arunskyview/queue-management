...
class ReportRepository
{
    ...
    public function getQueueListReport($starting_date, $ending_date)
    {
        $report = DB::table('queues')->Leftjoin('calls', 'calls.queue_id', '=', 'queues.id')
            ...
            if($user_id !=1)
            {
                $report=$report->where('users.id', $user_id);
            }
            $report=$report->select('services.name as service_name', 'queues.created_at as date', 'queues.number as token_number', 'queues.letter as token_letter', 'queues.called', 'users.name as user_name', 'counters.name as counter_name')
            ->get();
        return $report;
    }
